...
var aggregator = function(reduction, flushDelay, options) {
	...
	this.start = function(){
		if (options.debug) logger.info('Starting Aggregator with Agregation: ',reduction,'and Flush delay: ',flushDelay);
		options.fallback.upload(options.name, function(file,stream){
			if (options.debug) logger.info('Starting uploding',file)
			...
			stream.on('end', function() {
				...
				if (options.debug) logger.info('Upload done. Remove', file)
			});
		}.bind(this));
	}
	this.start()
	...
	this.ingest = function(id, stamp, key, value) {
		// if (options.debug) logger.info('Message received',id, stamp, key, value)
		...
		if(!this.buffer[r]) {
			this.buffer[r] = {
				...
				p: setTimeout(function() {
					if (options.debug) logger.info('Agregation is done for',this.index)
					...
					if (options.debug) logger.info('Removing backup file for',this.index)
					delete this.buffer[this.index];
				}.bind({buffer: this.buffer, index: r, parent: this}), reduction+flushDelay)
	...
	this.backup = function(timeSlot, id, stamp, key, value){
		if(!this.wStream[timeSlot]){
			if (options.debug) logger.info('Open write stream to',timeSlot)
		...
		options.protocol.encodeMsg(id, stamp, key, value, function(encoded) {
			// if (options.debug) logger.info('write',encoded,'to the',timeSlot)
			this.wStream[timeSlot].write(encoded + "\n");
		}.bind(this))
	}

	this.stop = function(){
		if (options.debug) logger.info('Stoping the aggregator')
		Object.keys(this.buffer).forEach(function(timeSlot) {
			clearTimeout(this.buffer[timeSlot].p);
			if (options.debug) logger.info('Timeslot '+timeSlot+' cleaned due to SIGTERM event')
		}.bind(this))
	}
