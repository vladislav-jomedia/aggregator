...
var aggregator = function(reduction, flushDelay, options) {
	...
	this.start = function(){
		if (options.debug === true){
			logger.info('Starting Aggregator with Agregation: ',reduction,'and Flush delay: ',flushDelay)
		}
		options.fallback.upload(options.name, function(file,stream){
			if (options.debug === true){
				logger.info('Starting uploding',file)
			}
			...
			stream.on('end', function() {
				...
				if (options.debug === true){
					logger.info('Upload done. Remove', file)
				}
			});
		}.bind(this));
	}
	...
	this.ingest = function(id, stamp, key, value) {
		...
		this.buffer[r].p = setTimeout(function() {
			if (options.debug === true){
				logger.info('Agregation is done for',this.index)
			}
			...
			if (options.debug === true){
				logger.info('Removing backup file for',this.index)
			}
	...
	this.backup = function(timeSlot, id, stamp, key, value){
		if(!this.wStream[timeSlot]){
			if (options.debug === true){
				logger.info('Open write stream to',timeSlot)
			}
	...
	this.stop = function(){
		if (options.debug === true){
			logger.info('Stoping the aggregator')
		}
