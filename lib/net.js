'use strict';
/**
 * New node file
 */
var reconnect = require('reconnect-net');
var reconnectTls = require('reconnect-tls');
var net = require('net');
var tls = require('tls');

module.exports = {
	socket: function(options) {
		if(!options.aggregator) {
			throw new Error('Aggregator is required');
		}
		if(!options.protocol) {
			throw new Error('Protocol is required');
		}
		if(!options.name || typeof(options.name) != 'string') {
			options.name = 'DefaultSocket'
		}

		var aggregator = options.aggregator;
		var protocol = options.protocol;
		var socketOption = (options.socketOption && typeof(options.socketOption) == 'object' ? options.socketOption : {});
		var chain = (options.chain ? options.chain: undefined);
		var logger = (options.logger ? options.logger: console);
		var plug;
		var connected = false;
		var handshake = false;

		var fallbackListenerAttached = false;
		var fallbackListener;
		var fallback;

		var write;

		if(options.fallback) {
			fallback = options.fallback;
			var wStream = 'false';
			fallbackListener = function(data) {
				if(wStream === 'false'){
					fallback.listener(options.name+'_'+'failback.txt',function(stream){
						wStream = stream;
					})
				}
				protocol.encode(data, function(encoded) {
					wStream.write(encoded + "\n");
				})
			};
			aggregator.on('data', fallbackListener);
		} else {
			fallback = undefined;
			fallbackListener = undefined;
		}

		return reconnect(socketOption, function(stream) {
			stream.on('data', function(data) {
				var msgs = String(data.toString()).split('\n');
				for(var x = 0; x < msgs.length; x++) {
					var msg = msgs[x];
					if(msg === "HELO") {
						logger.info('Handshake Received: ' + msg);
						handshake = true;
						if (fallbackListener) {
							aggregator.removeListener('data', fallbackListener);
							fallbackListenerAttached = false;
							logger.info('Removed Fallback listener');
							wStream = 'false'
							logger.info('Stopped/Removed write stream')
							fallback.flush(options.name+'_'+'failback.txt',function(stream) {
								stream.on('data', function(line) {
									write(line);
								});

								stream.on('end', function() {
									fallback.empty(options.name+'_'+'failback.txt',function(err) {
										if(err) {
											logger.error('Failed to remove backup file');
										} else {
											logger.info('Done flushing');
										}
									});
								});
							});
						}
						aggregator.on('data', plug);
					} else {
						if(msg.length) {
							logger.info('Received : ' + msg);
						}
					}
				}
			});
			stream.on('error', function(err) {
				logger.info(err)
			})
		}).on('connect', function(con) {

			write = function(line) {
				con.write(line + "\n");
				if(chain) {
					chain(line + "\n");
				}
			};

			logger.info('conn');
			connected = true;

			plug = function(data) {
				protocol.encode(data, function(encoded) {
                    write(encoded);
                    if (options.debug) logger.info('Sending => %s', encoded);
				});
			}

		}).on('reconnect', function(n, delay) {
			logger.info('Trying to re-conn');
		}).on('disconnect', function(err) {
			if(connected) {
				aggregator.removeListener('data', plug);
				logger.info('Disconnection');
				if (fallbackListener && !fallbackListenerAttached) {
					logger.info('Fallback Attached');
					aggregator.on('data', fallbackListener);
					fallbackListenerAttached = true;
				}
			}
			handshake = false;
			connected = false;
		});
	},
	server: function(options) {

		if(!options.aggregator || typeof(options.aggregator) != 'object') {
			throw new Error('Aggregator of type object must be provided');
		}
		if(!options.protocol || typeof(options.protocol) != 'object') {
			throw new Error('Protocol of type object must be provided');
		}
		if(!options.name || typeof(options.name) != 'string') {
			options.name = 'DefaultServer'
		}

		var aggregator = options.aggregator;
		var protocol = options.protocol;
		var name = (options.name ? options.name : 'Reducer Net Module Instance');
		var logger = (options.logger ? options.logger: console);

		return net.createServer(function(socket) {
			var chunk = null;
			socket.write('HELO\n');
			socket.write('Echo ' +name+ '\n');
			socket.on('data', function(encoded) {
				// If there is a chunk in memory, prepend it
				if(chunk != null) {
					encoded = chunk + encoded;
					chunk = null;
				}
				var msgs = encoded.toString().split('\n');	// Split resulting message
				if(msgs.length === 1) { // The whole message has no new lines. Storing in chunk until we get a full one.
					chunk = chunk + msgs[0];
				} else {
					var last = msgs.pop();
					if(last.length != 0) {
						chunk = last;
					}
					for(var x = 0; x < msgs.length; x++) {
						protocol.decode(msgs[x], function(err, id, stamp, key, value) {
							if(err) {
								logger.error(err);
								logger.error(id);
							} else {
								aggregator.ingest(id, stamp, key, value);
                                if (options.debug) logger.info('Ingesting => id [%s] stamp [%d] key [%s] value [%d]', id, stamp, key, value);
							}
						});
					}
				}
			});
			socket.on('error', function(err) {
				logger.error(err);
			});
		});
	},
	serverTls: function(options) {
		if(!options.authBase || typeof(options.authBase) != 'object') {
			throw new Error('authBase of type object must be provided');
		}
		if(!options.tlsServerOpt || typeof(options.tlsServerOpt) != 'object') {
			throw new Error('tlsServerOpt of type object must be provided');
		}
		if(!options.aggregator || typeof(options.aggregator) != 'object') {
			throw new Error('Aggregator of type object must be provided');
		}
		if(!options.protocol || typeof(options.protocol) != 'object') {
			throw new Error('Protocol of type object must be provided');
		}
		if(!options.name || typeof(options.name) != 'string') {
			options.name = 'DefaultServer'
		}

		var aggregator = options.aggregator;
		var protocol = options.protocol;
		var name = (options.name ? options.name : 'Reducer Net Module Instance');
		var logger = (options.logger ? options.logger: console);

		return tls.createServer(options.tlsServerOpt,function(socket) {
			var chunk = null;
			var authenticated = false;
			var namespace = null;
			socket.on('data', function(encoded) {
				if(chunk != null) {
					encoded = chunk + encoded;
					chunk = null;
				}
				var msgs = encoded.toString().split('\n');
				if(msgs.length === 1) {
					chunk = chunk + msgs[0];
				} else {
					var last = msgs.pop();
					if(last.length != 0) {
						chunk = last;
					}
					if (authenticated === false){
						console.log('Received Auth token: '+msgs[0])
						if (msgs[0] in options.authBase && options.authBase[msgs[0]]){
							authenticated = true;
							namespace = options.authBase[msgs[0]];
							socket.write('CONNECTED\n');
							console.log('Auth passed: '+namespace)
						}else{
							console.log('Auth failed, wrong token: ',msgs[0])
							socket.write('Access denied, wrong token\n');
						}
					}else{
						for(var x = 0; x < msgs.length; x++) {
							protocol.decode(msgs[x], function(err, id, stamp, key, value) {
								if(err) {
									// if ((id in options.authBase) === false){
										logger.error(err);
										logger.error(id);
									// }else{
										// logger.info('Dropping second auth')
									// }
								} else {
									aggregator.ingest(namespace+'__'+id, stamp, key, value);
	                                if (options.debug) logger.info('Ingesting => id [%s] stamp [%d] key [%s] value [%d]', namespace+'__'+id, stamp, key, value);
								}
							});
						}
					}
				}

			});
			socket.on('error', function(err) {
				logger.error(err);
			});
		});
	},
	clientTls: function(options) {
		if(!options.token) {
			throw new Error('Token is required');
		}
		if(!options.aggregator) {
			throw new Error('Aggregator is required');
		}
		if(!options.protocol) {
			throw new Error('Protocol is required');
		}
		if(!options.name || typeof(options.name) != 'string') {
			options.name = 'DefaultSocket'
		}

		var aggregator = options.aggregator;
		var protocol = options.protocol;
		var socketOption = (options.socketOption && typeof(options.socketOption) == 'object' ? options.socketOption : {});
		var chain = (options.chain ? options.chain: undefined);
		var logger = (options.logger ? options.logger: console);
		var plug;
		var connected = false;
		var authenticated = false;

		var fallbackListenerAttached = false;
		var fallbackListener;
		var fallback;

		var write;

		if(options.fallback) {
			fallback = options.fallback;
			var wStream = 'false';
			fallbackListener = function(data) {
				if(wStream === 'false'){
					fallback.listener(options.name+'_'+'failback.txt',function(stream){
						wStream = stream;
					})
				}
				protocol.encode(data, function(encoded) {
					wStream.write(encoded + "\n");
				})
			};
			aggregator.on('data', fallbackListener);
		} else {
			fallback = undefined;
			fallbackListener = undefined;
		}

		return reconnectTls(socketOption, function(stream) {
			stream.on('data', function(data) {
				var msgs = String(data.toString()).split('\n');
				if (authenticated === false){
					var authReply = msgs[0];
					console.log('Received Auth reply: '+authReply)
					if (authReply === 'CONNECTED'){
						authenticated = true;
						console.log('Auth passed')
						if (fallbackListener) {
							aggregator.removeListener('data', fallbackListener);
							fallbackListenerAttached = false;
							logger.info('Removed Fallback listener');
							wStream = 'false'
							logger.info('Stopped/Removed write stream')
							fallback.flush(options.name+'_'+'failback.txt',function(stream) {
								stream.on('data', function(line) {
									write(line);
								});

								stream.on('end', function() {
									fallback.empty(options.name+'_'+'failback.txt',function(err) {
										if(err) {
											logger.error('Failed to remove backup file');
										} else {
											logger.info('Done flushing');
										}
									});
								});
							});
						}
						aggregator.on('data', plug);
					}
				}
			});
			stream.on('error', function(err) {
				console.log(err)
			})
		}).on('connect', function(con) {
			console.log('ON Connect')
			con.write(options.token+'\n');

			write = function(line) {
				con.write(line + "\n");
				if(chain) {
					chain(line + "\n");
				}
			};

			logger.info('conn');
			connected = true;

			plug = function(data) {
				protocol.encode(data, function(encoded) {
                    write(encoded);
                    if (options.debug) logger.info('Sending => %s', encoded);
				});
			}
		}).on('reconnect', function(n, delay) {
			logger.info('Trying to re-conn');
		}).on('disconnect', function(err) {
			console.log('ON DISConnect: '+err)
			authenticated = false;
			if(connected) {
				aggregator.removeListener('data', plug);
				logger.info('Disconnection');
				if (fallbackListener && !fallbackListenerAttached) {
					logger.info('Fallback Attached');
					aggregator.on('data', fallbackListener);
					fallbackListenerAttached = true;
				}
			}
			connected = false;
		}).on('error', function(err) {
			console.log(err)
		})
	}
};

